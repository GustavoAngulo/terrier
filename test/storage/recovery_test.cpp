  ...
  void SetUp() override {
    ...
    log_manager_ =
        new LogManager(LOG_FILE_NAME, num_log_buffers_, log_serialization_interval_, log_persist_interval_,
                       log_persist_threshold_, "", 0, false, &buffer_pool_, common::ManagedPointer(thread_registry_));
    log_manager_->Start();
...
TEST_F(RecoveryTests, DoubleRecoveryTest) {
  ...
  RecoveryManager secondary_recovery_manager(
      common::ManagedPointer<storage::AbstractLogProvider>(&secondary_log_provider),
      common::ManagedPointer(&secondary_recovery_catalog), &secondary_recovery_txn_manager,
