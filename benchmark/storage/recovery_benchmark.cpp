...
class RecoveryBenchmark : public benchmark::Fixture {
 public:
  ...
  const std::chrono::milliseconds metrics_overhead_threshold_{1000000000};
  ...
  void RunBenchmark(benchmark::State *state, const LargeSqlTableTestConfiguration &config) {
    ...
    for (auto _ : *state) {
      ...
      metrics::MetricsThread metrics_thread(metrics_period_);
      for (const auto component : metrics_components_) {
        metrics_thread.GetMetricsManager().EnableMetric(component);
      }
      thread_registry_ = new common::DedicatedThreadRegistry(common::ManagedPointer(&(metrics_thread.GetMetricsManager())));
      storage::LogManager log_manager(LOG_FILE_NAME, num_log_buffers_, log_serialization_interval_,
                                      ...
                                      false, &buffer_pool_, common::ManagedPointer(thread_registry_));
      log_manager.Start();
      ...
      storage::RecoveryManager recovery_manager(common::ManagedPointer<storage::AbstractLogProvider>(&log_provider),
                                                ...
                                                DISABLED, metrics_overhead_polling_interval_, metrics_overhead_threshold_);
...
//                                    log_persist_threshold_, "" /* replication disabled */, 0, false, &buffer_pool_,
...
//                                              &recovery_deferred_action_manager,
//                                              common::ManagedPointer(thread_registry_), &block_store_,
//                                              common::ManagedPointer(&metrics_thread_), metrics_overhead_polling_interval_,
