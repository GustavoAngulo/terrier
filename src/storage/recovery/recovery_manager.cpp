  ...
  if (txns_processed > 0) {
    ...
    if (IsAsynchronousReplication()) {
      for (auto txn_id : committed_txns_) {
        ...
        TERRIER_ASSERT(search != raw_commit_time_.end(), "Raw commit should have been added already");
        auto async_replication_delay_ns =
            std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now() - search->second)
                .count();
    ...
    if (next_record->RecordType() == LogRecordType::REDO) {  // next record is a redo record
      ...
      if (next_redo_record->GetDatabaseOid() == delete_record->GetDatabaseOid() &&
        ...
        if (db_oid == next_db_oid) {
          ...
          VarlenEntry name_varlen = *(reinterpret_cast<VarlenEntry *>(
              next_redo_record->Delta()->AccessWithNullCheck(pr_map[catalog::postgres::DATNAME_COL_OID])));
          std::string name_string(name_varlen.StringView());
...
common::ManagedPointer<storage::SqlTable> RecoveryManager::GetSqlTable(transaction::TransactionContext *txn,
  ...
  if (table_oid == catalog::postgres::DATABASE_TABLE_OID) {
